...
if selected_year != "All":
    df = df[df['order_purchase_timestamp'].dt.year == int(selected_year)]

if selected_state:
    df = df[df['customer_state'].isin(selected_state)]

st.sidebar.divider()
st.sidebar.metric("Filtered Orders", f"{df['order_id'].nunique():,}")
st.sidebar.metric("Filtered Revenue", f"R${df['payment_value'].sum():,.0f}")
